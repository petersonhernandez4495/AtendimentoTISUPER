...
class _DetalhesChamadoScreenState extends State<DetalhesChamadoScreen> {
  ...
  Future<String?> _getSignatureUrlFromFirestore(String? userId) async {
    ...
    try {
      ...
      print(
          "DetalhesChamadoScreen: Erro ao buscar URL da assinatura para $userId: $e");
    }
    return null;
  }
  ...
  Future<void> _mostrarDialogoEdicao(Map<String, dynamic> dadosAtuais) async {
    ...
    String statusSelecionado =
        dadosAtuais[kFieldStatus] as String? ?? _listaStatusEdicao.first;
    String prioridadeSelecionada =
        dadosAtuais[kFieldPrioridade] as String? ?? _listaPrioridades.first;
    String tecnicoResponsavel =
        dadosAtuais[kFieldTecnicoResponsavel] as String? ?? '';
    String tecnicoUid = dadosAtuais[kFieldTecnicoUid] as String? ?? '';

    final solutionControllerDialog = TextEditingController(
        text: dadosAtuais[kFieldSolucao] as String? ?? '');
    bool showMandatoryFields = statusSelecionado.toLowerCase() ==
        kStatusPadraoSolicionado.toLowerCase();
    DateTime? selectedAtendimentoDate;
    final Timestamp? currentAtendimentoTs =
        dadosAtuais[kFieldDataAtendimento] as Timestamp?;
    if (currentAtendimentoTs != null) {
      selectedAtendimentoDate = currentAtendimentoTs.toDate();
    }

    if (!_listaStatusEdicao.contains(statusSelecionado))
      statusSelecionado = _listaStatusEdicao.first;
    if (!_listaPrioridades.contains(prioridadeSelecionada))
      prioridadeSelecionada = _listaPrioridades.first;

    Future<void> selectDate(
        BuildContext dlgContext, Function(DateTime?) onDateSelected) async {
      final DateTime? picked = await showDatePicker(
        context: dlgContext,
        initialDate: selectedAtendimentoDate ?? DateTime.now(),
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    DropdownButtonFormField<String>(
                      ...
                      onChanged: (newValue) {
                        if (newValue != null) {
                          setDialogState(() {
                            ...
                            showMandatoryFields = newValue.toLowerCase() ==
                                kStatusPadraoSolicionado.toLowerCase();
                            if (!showMandatoryFields) {
                              solutionControllerDialog.clear();
                              selectedAtendimentoDate = null;
                            }
                          });
                        }
                      },
                      ...
                      validator: (v) => v == null ? 'Selecione status' : null,
                    ),
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      validator: (v) =>
                          v == null ? 'Selecione prioridade' : null,
                    ),
                    const SizedBox(height: kSpacingSmall),
                    TextFormField(
                      initialValue: tecnicoResponsavel,
                      onChanged: (value) => tecnicoResponsavel = value,
                      decoration: const InputDecoration(
                          labelText: 'Técnico Responsável (Nome)',
                          isDense: true),
                    ),
                    const SizedBox(height: kSpacingXSmall),
                    TextFormField(
                      initialValue: tecnicoUid,
                      onChanged: (value) => tecnicoUid = value,
                      decoration: const InputDecoration(
                          labelText: 'UID do Técnico (Opcional)',
                          isDense: true),
                    ),
                    const SizedBox(height: kSpacingSmall),
                    Visibility(
                        ...
                        child: Column(
                            ...
                            children: [
                              ...
                              Text("Detalhes da Solução (Obrigatório)",
                                  style: theme.textTheme.labelLarge
                                      ?.copyWith(fontWeight: FontWeight.bold)),
                              const SizedBox(height: kSpacingXSmall),
                              TextFormField(
                                controller: solutionControllerDialog,
                                decoration: const InputDecoration(
                                ...
                                validator: (value) {
                                  if (showMandatoryFields &&
                                    ...
                                    return 'Descrição obrigatória para solucionar.';
                                  }
                                  return null;
                                },
                              ),
                              const SizedBox(height: kSpacingSmall),
                              Text("Data de Atendimento (Obrigatório)",
                              ...
                              Row(
                                children: [
                                    ...
                                    child: OutlinedButton.icon(
                                      ...
                                      label: Text(
                                        selectedAtendimentoDate == null
                                            ? 'Selecionar Data'
                                            : dateFormat.format(
                                                selectedAtendimentoDate!),
                                      ),
                                      onPressed: () {
                                        selectDate(dialogContext, (pickedDate) {
                                          setDialogState(() {
                                            selectedAtendimentoDate =
                                                pickedDate;
                                          });
                                  ...
                                  if (selectedAtendimentoDate != null)
                                    IconButton(
                                      ...
                                      padding:
                                          const EdgeInsets.all(kSpacingXSmall),
                                    ),
                                ],
                              ),
                              const SizedBox(height: kSpacingSmall),
            ...
            actions: <Widget>[
              ...
              ElevatedButton(
                ...
                onPressed: () {
                  ...
                  if (statusSelecionado.toLowerCase() ==
                      kStatusPadraoSolicionado.toLowerCase()) {
                    final isDescriptionEmpty =
                        solutionControllerDialog.text.trim().isEmpty;
                    final isDateMissing = selectedAtendimentoDate == null;
                    if (isDescriptionEmpty || isDateMissing) {
                      ScaffoldMessenger.of(dialogContext)
                          .showSnackBar(const SnackBar(
                        content: Text(
                            'Para status "Solucionado", descrição e data são obrigatórias.'),
    ...
    if (confirmou == true && mounted) {
      ...
      if (usuarioLogado == null) {
        if (mounted)
          scaffoldMessenger.showSnackBar(const SnackBar(
            content: Text('Erro: Sessão de usuário inválida.'),
      ...
      try {
        ...
        final String? solucaoFinal = statusSelecionado.toLowerCase() ==
                kStatusPadraoSolicionado.toLowerCase()
            ? solutionControllerDialog.text.trim()
            : null;
        final Timestamp? atendimentoTimestamp = selectedAtendimentoDate != null
            ? Timestamp.fromDate(selectedAtendimentoDate!)
            : null;
  ...
  Future<void> _handlePdfShare(Map<String, dynamic> currentData) async {
    ...
    final String? adminSolucionouUid =
        currentData[kFieldSolucaoPorUid] as String?;
    ...
    final bool requerenteConfirmou =
        currentData[kFieldRequerenteConfirmou] as bool? ?? false;
    final String? uidDoRequerenteQueConfirmou =
        currentData[kFieldRequerenteConfirmouUid] as String?;
    if (requerenteConfirmou &&
        uidDoRequerenteQueConfirmou != null &&
        uidDoRequerenteQueConfirmou.isNotEmpty) {
      requesterSigUrl =
          await _getSignatureUrlFromFirestore(uidDoRequerenteQueConfirmou);
    }
    ...
    try {
      ...
      if (mounted) {
        scaffoldMessenger.showSnackBar(
          SnackBar(
              content: Text('Erro ao gerar PDF para compartilhamento: $e'),
              backgroundColor: Colors.red),
        );
      }
      return;
    ...
    if (mounted) {
      try {
        await Printing.sharePdf(
            bytes: pdfBytes,
            filename: 'chamado_${widget.chamadoId.substring(0, 6)}.pdf');
      } catch (e) {
        if (mounted) {
          scaffoldMessenger.showSnackBar(
            SnackBar(
      ...
      scaffoldMessenger.showSnackBar(
        const SnackBar(
            content: Text('Falha ao gerar PDF para compartilhamento.'),
            backgroundColor: Colors.orange),
      );
    }
  }
  ...
  Future<void> _baixarPdf(Map<String, dynamic> dadosChamado) async {
    ...
    Uint8List? pdfBytes;
    try {
      // CARREGAR IMAGENS DOS ASSETS AQUI
      Uint8List? logoGovAssetBytes;
      Uint8List? logoEmblemAssetBytes;

      try {
        final ByteData logoGovData = await rootBundle.load(
            'assets/images/FOTO-PERFIL_ROLIM-DE-MOURA.jpg'); // Certifique-se que o caminho está correto no seu pubspec.yaml e no projeto
        logoGovAssetBytes = logoGovData.buffer.asUint8List();
      } catch (e) {
        print("Erro ao carregar logoGovBytes dos assets: $e");
        // Você pode querer mostrar um erro ou usar uma imagem padrão
      }

      try {
        final ByteData logoEmblemData = await rootBundle.load(
            'assets/images/seu_logo.jpg'); // Certifique-se que o caminho está correto
        logoEmblemAssetBytes = logoEmblemData.buffer.asUint8List();
      } catch (e) {
        print("Erro ao carregar logoEmblemAssetBytes dos assets: $e");
      }

      // Obter URLs de assinatura (seu código existente)
      String? adminSigUrl;
      final String? adminSolucionouUid =
          dadosChamado[kFieldSolucaoPorUid] as String?;
      if (adminSolucionouUid != null && adminSolucionouUid.isNotEmpty) {
        adminSigUrl = await _getSignatureUrlFromFirestore(adminSolucionouUid);
      }
      ...
      final String? uidDoRequerenteQueConfirmou =
          dadosChamado[kFieldRequerenteConfirmouUid] as String?;
      if (requerenteConfirmou &&
          uidDoRequerenteQueConfirmou != null &&
          uidDoRequerenteQueConfirmou.isNotEmpty) {
        requesterSigUrl =
      ...
      // Gerar o PDF passando os bytes das imagens carregadas
      pdfBytes = await pdfGen.PdfGenerator.generateTicketPdfBytes(
        chamadoId: widget.chamadoId,
        dadosChamado: dadosChamado,
        adminSignatureUrl: adminSigUrl,
        requesterSignatureUrl: requesterSigUrl,
        logoGovBytes: logoGovAssetBytes, // Passa os bytes carregados
        logoEmblemBytes: logoEmblemAssetBytes, // Passa os bytes carregados
      );
    } catch (e) {
      if (Navigator.of(currentContext, rootNavigator: true).canPop()) {
        Navigator.of(currentContext, rootNavigator: true).pop();
    ...
    if (mounted) {
      showDialog(
        context:
            context, // Usa o contexto original do State para mostrar o novo diálogo
        builder: (BuildContext dialogContext) {
          return AlertDialog(
      ...
      scaffoldMessenger.showSnackBar(
        const SnackBar(
            content: Text('Falha ao gerar PDF.'),
            backgroundColor: Colors.orange),
      );
    }
  }
  ...
  Future<void> _imprimirPdf(Uint8List pdfBytes) async {
    ...
    final scaffoldMessenger =
        ScaffoldMessenger.of(context); // Adicionado para mostrar erros
    try {
  ...
  Future<void> _abrirPdfLocalmente(Uint8List pdfBytes, String chamadoId) async {
    if (!mounted) return;
    final scaffoldMessenger = ScaffoldMessenger.of(context);
    BuildContext currentContextForDialog =
        context; // Salva contexto para diálogo de progresso
  ...
  Future<void> _adicionarComentario() async {
    ...
    if (u == null) {
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Faça login para adicionar um comentário.')));
      return;
    }
    setState(() {
    ...
    try {
      final a = u.displayName?.trim().isNotEmpty ?? false
          ? u.displayName!.trim()
          : (u.email ?? "Usuário Desconhecido");
  ...
  Future<void> _toggleInatividadeAdmin(bool isInativoAtual) async {
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            ...
            child: Text(acao,
                style:
                    TextStyle(color: deveAtivar ? Colors.green : Colors.red)),
          ),
        ],
      ),
    );
    ...
    try {
      await _chamadoService.definirInatividadeAdministrativa(
          widget.chamadoId, !isInativoAtual);
      if (mounted) {
        scaffoldMessenger.showSnackBar(SnackBar(
  ...
  Future<void> _adminFinalizarChamado(
      Map<String, dynamic> dadosChamadoExibido) async {
    if (!_isAdmin || !mounted) return;
    final User? adminUser = _auth.currentUser;
    if (adminUser == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content:
              Text('Usuário administrador não autenticado para esta ação.')));
      return;
    }
    final bool requerenteJaConfirmou =
        dadosChamadoExibido[kFieldRequerenteConfirmou] as bool? ?? false;
    if (!requerenteJaConfirmou) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content:
              Text('Aguardando confirmação do requerente antes de arquivar.'),
          backgroundColor: Colors.orange));
      return;
    }
    final String statusAtual =
        dadosChamadoExibido[kFieldStatus] as String? ?? '';
    if (statusAtual.toLowerCase() != kStatusPadraoSolicionado.toLowerCase()) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text(
              'O chamado precisa estar "$kStatusPadraoSolicionado" para ser arquivado.'),
          backgroundColor: Colors.orange));
      return;
    }
    final bool adminJaFinalizou =
        dadosChamadoExibido[kFieldAdminFinalizou] as bool? ?? false;
    if (adminJaFinalizou) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Este chamado já foi arquivado.'),
    ...
    bool confirmarAdmin = await showDialog<bool>(
          ...
          builder: (ctx) => AlertDialog(
            ...
            content: const Text(
                'Deseja mover este chamado para o arquivo? Esta ação registrará sua identificação como o responsável pelo arquivamento.'),
            actions: [
              TextButton(
                  child: const Text('Cancelar'),
                  onPressed: () => Navigator.of(ctx).pop(false)),
              ElevatedButton(
                  child: const Text('Confirmar e Arquivar'),
                  onPressed: () => Navigator.of(ctx).pop(true)),
            ],
          ),
        ) ??
    ...
    if (confirmarAdmin && mounted) {
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: Text('Erro ao arquivar chamado: ${e.toString()}'),
              backgroundColor: Colors.red));
        }
      } finally {
        if (mounted) {
  ...
  Widget build(BuildContext context) {
    if (!_isAdminStatusChecked) {
      return Scaffold(
        appBar: AppBar(
            title: Text(
                'Chamado #${widget.chamadoId.substring(0, min(6, widget.chamadoId.length))}...')),
              ...
              builder: (context, snapshot) {
                if (snapshot.hasData && snapshot.data!.exists) {
                  ...
                  final bool isInativoAdmin =
                      currentData[kFieldAdminInativo] ?? false;
                    ...
                    children: [
                      if (_isAdmin)
                        IconButton(
                          ...
                          onPressed:
                              _isUpdatingVisibility || _isFinalizandoAdmin
                                  ? null
                                  : () => _mostrarDialogoEdicao(currentData),
                        ),
                      if (podeInteragir) ...[
                        IconButton(
        ...
        children: [
          Expanded(
            child: StreamBuilder<DocumentSnapshot>(
              ...
              builder: (context, snapshotChamado) {
                if (snapshotChamado.hasError) {
                  return Center(child: Text('Erro: ${snapshotChamado.error}'));
                }
                if (snapshotChamado.connectionState ==
                    ConnectionState.waiting) {
                  return const Center(child: CircularProgressIndicator());
                }
                if (!snapshotChamado.hasData || !snapshotChamado.data!.exists) {
                  return const Center(child: Text('Chamado não encontrado.'));
                }

                final Map<String, dynamic> data =
                    snapshotChamado.data!.data()! as Map<String, dynamic>;
                final bool isInativoAdmin = data[kFieldAdminInativo] ?? false;

                return _ChamadoInfoBody(
                  key: ValueKey(widget.chamadoId +
                      (data[kFieldDataAtualizacao]?.toString() ??
          ...
          StreamBuilder<DocumentSnapshot>(
              ...
              builder: (context, snapshot) {
                ...
                final bool isInativoAdmin = (snapshot.data!.data()
                        as Map<String, dynamic>?)?[kFieldAdminInativo] ??
                    false;
                ...
                return AnimatedSwitcher(
                  ...
                  child: podeComentar
                      ...
                      : Container(
                          ...
                          padding: const EdgeInsets.all(kSpacingSmall).copyWith(
      ...
      builder: (context, snapshotVisitas) {
        if (snapshotVisitas.hasError) {
          return Padding(
              ...
              child: Text("Erro: ${snapshotVisitas.error}"));
        }
        ...
        return Column(
          children: snapshotVisitas.data!.docs.map((docVisita) {
            ...
            final String dtHr = ts != null
                ? DateFormat('dd/MM/yy HH:mm', 'pt_BR').format(ts.toDate())
                : 'Data Inválida';
            ...
            return Card(
              margin: const EdgeInsets.symmetric(
                  vertical: kSpacingXSmall, horizontal: 0),
              elevation: 1,
              shape: RoundedRectangleBorder(
                  borderRadius: BorderRadius.circular(4)),
              child: ListTile(
                leading: Icon(_getVisitaStatusIcon(st),
                    color: _getVisitaStatusColor(st), size: 26),
                title: Text("Agendado: $dtHr",
                    style: Theme.of(context)
                        .textTheme
                        .bodyLarge
                ...
                contentPadding: const EdgeInsets.symmetric(
                    vertical: kSpacingXSmall, horizontal: kSpacingSmall),
              ),
            );
          }).toList(),
        );
      ...
      builder: (context, snapshot) {
        ...
        if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
          return const Padding(
            padding: EdgeInsets.symmetric(vertical: kSpacingSmall),
            child: Center(
                child: Text("Nenhum comentário ou histórico.",
                    style:
                        TextStyle(fontStyle: FontStyle.italic, fontSize: 13))),
          );
        }
            ...
            return Card(
              ...
              child: ListTile(
                ...
                subtitle: Padding(
                    ...
                    child: Text(sys ? "Sistema - $dtHr" : "$a - $dtHr",
                ...
                dense: true,
                contentPadding: const EdgeInsets.symmetric(
                    vertical: kSpacingXSmall - 2, horizontal: kSpacingSmall),
              ),
            );
          }).toList(),
        );
  ...
  Widget _buildCommentInputArea() {
    ...
    return Container(
      ...
      padding: const EdgeInsets.symmetric(
              horizontal: kSpacingSmall, vertical: kSpacingXSmall)
          .copyWith(
              bottom: MediaQuery.of(context).padding.bottom + kSpacingXSmall),
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
        Expanded(
          child: TextField(
            ...
            onSubmitted: (_) =>
                _isSendingComment ? null : _adicionarComentario(),
...
class _ChamadoInfoBody extends StatelessWidget {
  ...
  Widget _buildSectionTitle(BuildContext context, String title,
      {Color? titleColor, bool addTopPadding = true}) {
  ...
  Widget _buildInfoRow(BuildContext context,
    ...
    final String displayValue =
        value?.trim().isEmpty ?? true ? '--' : value!.trim();
  ...
  Widget _buildStatusChips(BuildContext context,
      {required String status, required String prioridade}) {
    final ThemeData t = Theme.of(context);
    final TextTheme tt = t.textTheme;
    ...
    final Color priorityColor =
        AppTheme.getPriorityColor(prioridade) ?? t.colorScheme.secondary;

    return Padding(
      padding:
          const EdgeInsets.only(top: kSpacingXSmall - 2, bottom: kSpacingSmall),
      child: Wrap(
        ...
        children: [
          Chip(
            ...
            avatar: Icon(Icons.flag_circle_outlined,
                size: 15,
                color: getChipTextColor(statusColor).withOpacity(0.85)),
            padding: const EdgeInsets.symmetric(
                horizontal: kSpacingSmall - 2, vertical: kSpacingXXSmall + 1),
          ...
          Chip(
            ...
            avatar: Icon(Icons.label_important_outline_rounded,
                size: 15,
                color: getChipTextColor(priorityColor).withOpacity(0.85)),
            padding: const EdgeInsets.symmetric(
                horizontal: kSpacingSmall - 2, vertical: kSpacingXXSmall + 1),
            visualDensity: VisualDensity.compact,
            materialTapTargetSize: MaterialTapTargetSize.shrinkWrap,
  ...
  String formatTimestampSafe(Timestamp? ts,
  ...
  Widget build(BuildContext context) {
    ...
    final String tipoSolicitante =
        data[kFieldTipoSolicitante]?.toString() ?? 'N/I';
    final String nomeSolicitante =
        data[kFieldNomeSolicitante]?.toString() ?? 'N/I';
    final String celularContato =
        data[kFieldCelularContato]?.toString() ?? 'N/I';
    final String equipamentoSolicitacao =
        data[kFieldEquipamentoSolicitacao]?.toString() ?? 'N/I';
    final String conectadoInternet =
        data[kFieldConectadoInternet]?.toString() ?? 'N/I';
    ...
    final String problemaOcorre =
        data[kFieldProblemaOcorre]?.toString() ?? 'N/I';
    ...
    final String? tecnicoResponsavel =
        data[kFieldTecnicoResponsavel] as String?;
    final String? authUserDisplay = data[kFieldAuthUserDisplay] as String?;
    final String dtCriacao =
        formatTimestampSafe(data[kFieldDataCriacao] as Timestamp?);
    final Timestamp? tsAtendimento = data[kFieldDataAtendimento] as Timestamp?;
    final String dtAtendimento =
        formatTimestampSafe(tsAtendimento, format: 'dd/MM/yyyy');
    ...
    final String? cidadeSuperintendencia =
        data[kFieldCidadeSuperintendencia] as String?;
    final String? instituicaoManual = data[kFieldInstituicaoManual] as String?;
    final String? equipamentoOutroDesc =
        data[kFieldEquipamentoOutro] as String?;
    final String? problemaOutroDesc = data[kFieldProblemaOutro] as String?;
    final String? solucao = data[kFieldSolucao] as String?;
    final bool requerenteConfirmou =
        data[kFieldRequerenteConfirmou] as bool? ?? false;
    final String dtConfirmacaoReq =
        formatTimestampSafe(data[kFieldRequerenteConfirmouData] as Timestamp?);
    final String? nomeRequerenteConfirmador = data[
            kFieldNomeRequerenteConfirmador] as String? ??
        (data[kFieldRequerenteConfirmouUid] != null
            ? 'Solicitante (UID: ${(data[kFieldRequerenteConfirmouUid] as String).substring(0, min(6, (data[kFieldRequerenteConfirmouUid] as String).length))}...)'
            : null);
    final bool adminFinalizou = data[kFieldAdminFinalizou] as bool? ?? false;
    final String? adminFinalizouNome =
        data[kFieldAdminFinalizouNome] as String?;
    final String adminFinalizouDataStr =
        formatTimestampSafe(data[kFieldAdminFinalizouData] as Timestamp?);

    final String? solucionadoPorNome = data[kFieldSolucaoPorNome] as String?;
    final String dataDaSolucaoStr =
        formatTimestampSafe(data[kFieldDataDaSolucao] as Timestamp?);
    ...
    if (equipamentoSolicitacao == "OUTRO" &&
    ...
    Widget solicitacaoSection = Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildSectionTitle(context, 'Dados da Solicitação',
            addTopPadding: false),
        ...
        if (tipoSolicitante == 'ESCOLA') ...[
          if (cidade != null)
            _buildInfoRow(context, label: 'Cidade/Distrito', value: cidade),
          _buildInfoRow(context,
              label: 'Instituição',
              value: displayInstituicao,
              valueMaxLines: 3),
          if (cargoFuncao != null)
            _buildInfoRow(context, label: 'Cargo/Função', value: cargoFuncao),
          if (atendimentoPara != null)
            _buildInfoRow(context,
                label: 'Atendimento Para', value: atendimentoPara),
        ],
        if (tipoSolicitante == 'SUPERINTENDENCIA') ...[
          if (setorSuper != null)
            _buildInfoRow(context,
                label: 'Setor SUPER', value: setorSuper, valueMaxLines: 3),
          if (cidadeSuperintendencia != null)
            _buildInfoRow(context,
                label: 'Cidade SUPER', value: cidadeSuperintendencia),
        ],
      ],
    );
    ...
    Widget problemaDatasSection = Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildSectionTitle(context, 'Detalhes do Problema',
            addTopPadding: false),
        _buildInfoRow(context,
            label: 'Problema', value: displayProblema, valueMaxLines: 4),
        _buildInfoRow(context,
            label: 'Equipamento', value: displayEquipamento, valueMaxLines: 3),
        if (marcaModelo.isNotEmpty)
          _buildInfoRow(context, label: 'Marca/Modelo', value: marcaModelo),
        ...
        if (tecnicoResponsavel != null && tecnicoResponsavel.isNotEmpty)
          _buildInfoRow(context,
              label: 'Técnico Resp.', value: tecnicoResponsavel),
        ...
        if (authUserDisplay != null && authUserDisplay.isNotEmpty)
          _buildInfoRow(context,
              label: 'Registrado por', value: authUserDisplay),
      ],
    );
        ...
        children: <Widget>[
          if (isInativoAdmin)
            Padding(
              ...
              child: Center(
                child: Chip(
                  label: const Text('CHAMADO INATIVO (APENAS ADMINS VÊEM)',
                      style: TextStyle(
                          color: Colors.white,
                          fontWeight: FontWeight.bold,
                          fontSize: 11)),
                  ...
                  padding: const EdgeInsets.symmetric(
                      horizontal: kSpacingSmall, vertical: kSpacingXSmall - 2),
                ),
              ),
            ),
          ...
          if (solucao != null && solucao.isNotEmpty) ...[
            ...
            if (solucionadoPorNome != null && dataDaSolucaoStr != '--') ...[
              _buildInfoRow(context,
                  label: 'Solucionado por', value: solucionadoPorNome),
              _buildInfoRow(context,
                  label: 'Data da Solução', value: dataDaSolucaoStr),
            ],
            Container(
              padding: const EdgeInsets.all(kSpacingSmall - 2),
              ...
              child: SelectableText(solucao,
                  style: textTheme.bodyMedium?.copyWith(height: 1.35)),
            ),
            const SizedBox(height: kSpacingSmall),
          ],
          if (requerenteConfirmou) ...[
            _buildSectionTitle(context, 'Confirmação do Requerente',
                titleColor: Colors.green.shade700),
            _buildInfoRow(context,
                label: 'Status Confirmação',
                value: 'Solução Aceita pelo Requerente'),
            if (nomeRequerenteConfirmador != null)
              _buildInfoRow(context,
                  label: 'Confirmado por', value: nomeRequerenteConfirmador),
            _buildInfoRow(context,
                label: 'Data Confirmação', value: dtConfirmacaoReq),
            const SizedBox(height: kSpacingSmall),
          ],
          if (isAdmin) ...[
            if (podeAdminFinalizar) ...[
              ...
              _buildSectionDivider(context),
              _buildSectionTitle(context, 'Detalhes do Arquivamento',
                  titleColor: AppTheme.kWinStatusFinalizadoBackground),
              _buildInfoRow(context,
                  label: 'Data Arquivamento', value: adminFinalizouDataStr),
              if (adminFinalizouNome != null)
                _buildInfoRow(context,
                    label: 'Arquivado por', value: adminFinalizouNome),
              _buildInfoRow(context,
                  label: 'Status Sistema', value: 'Chamado Arquivado'),
              const SizedBox(height: kSpacingSmall),
            ],
          ],
            ...
            Center(
              child: ElevatedButton.icon(
                icon: Icon(
                    ...
                    size: 18),
                label: Text(
                    isInativoAdmin ? "Reativar p/ Requ." : "Inativar p/ Requ."),
                style: ElevatedButton.styleFrom(
                  backgroundColor: isInativoAdmin
                      ? colorScheme.primary
                ...
                TextButton.icon(
                  ...
                  onPressed: () {
                    Navigator.push(
                      ...
                      MaterialPageRoute(
                        builder: (context) =>
                            AgendamentoVisitaScreen(chamadoId: chamadoId),
                      ),
                    );
                  },
                ),
          ...
          Padding(
            ...
            child: Text("Comentários e Histórico",
                style: textTheme.titleMedium
                    ?.copyWith(fontWeight: FontWeight.w600)),
          ),
