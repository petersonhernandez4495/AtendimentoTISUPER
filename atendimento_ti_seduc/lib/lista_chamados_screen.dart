...
class _ListaChamadosScreenState extends State<ListaChamadosScreen> {
  ...
  Query _buildFirestoreQuery() { Query query = FirebaseFirestore.instance.collection('chamados'); if (_selectedStatusFilter != null && _selectedStatusFilter != 'Todos') { query = query.where('status', isEqualTo: _selectedStatusFilter); } if (_selectedDateFilter != null) { final DateTime startOfDay = DateTime(_selectedDateFilter!.year, _selectedDateFilter!.month, _selectedDateFilter!.day, 0, 0, 0); final DateTime endOfDay = DateTime(_selectedDateFilter!.year, _selectedDateFilter!.month, _selectedDateFilter!.day, 23, 59, 59); final Timestamp startTimestamp = Timestamp.fromDate(startOfDay); final Timestamp endTimestamp = Timestamp.fromDate(endOfDay); query = query.where('data_criacao', isGreaterThanOrEqualTo: startTimestamp); query = query.where('data_criacao', isLessThanOrEqualTo: endTimestamp); } query = query.orderBy(_sortField, descending: _sortDescending); if (_sortField != 'data_criacao' && _selectedDateFilter == null) { query = query.orderBy('data_criacao', descending: true); } else if (_sortField != 'data_criacao' && _selectedDateFilter != null) { } return query; }
  Future<void> _excluirChamado(BuildContext context, String chamadoId) async { bool confirmarExclusao = await showDialog<bool>( context: context, builder: (BuildContext context) { return AlertDialog( title: const Text('Confirmar Exclusão'), content: const Text( 'Tem certeza que deseja excluir este chamado?\nEsta ação não pode ser desfeita.'), actions: <Widget>[ TextButton( child: const Text('Cancelar'), onPressed: () { Navigator.of(context).pop(false); }, ), TextButton( child: Text('Excluir', style: TextStyle(color: AppTheme.kErrorColor)), onPressed: () { Navigator.of(context).pop(true); }, ), ], ); } ) ?? false; if (!confirmarExclusao || !mounted) return; try { await FirebaseFirestore.instance.collection('chamados').doc(chamadoId).delete(); if (mounted) { ScaffoldMessenger.of(context).showSnackBar( const SnackBar(content: Text('Chamado excluído com sucesso!')), ); } } catch (error) { print('Erro ao excluir chamado: $error'); if (mounted) { ScaffoldMessenger.of(context).showSnackBar( SnackBar(content: Text('Erro ao excluir chamado: ${error.toString()}')), ); } } }
  Future<void> _gerarECompartilharPdfLista() async { if (_currentDocs == null || _currentDocs!.isEmpty) { if(mounted) ScaffoldMessenger.of(context).showSnackBar( const SnackBar(content: Text('Nenhum chamado na lista atual para gerar PDF.')) ); return; } showDialog( context: context, barrierDismissible: false, builder: (_) => const Center(child: CircularProgressIndicator()) ); try { final Uint8List pdfBytes = await generateTicketListPdf(_currentDocs!); final Directory tempDir = await getTemporaryDirectory(); final String filePath = '${tempDir.path}/lista_chamados_${DateFormat('yyyyMMdd_HHmm').format(DateTime.now())}.pdf'; final File file = File(filePath); await file.writeAsBytes(pdfBytes); if (!mounted) return; Navigator.of(context, rootNavigator: true).pop(); final result = await Share.shareXFiles( [XFile(filePath)], text: 'Lista de Chamados - ${DateFormat('dd/MM/yyyy HH:mm').format(DateTime.now())}' ); if (result.status == ShareResultStatus.success && mounted) { print("Compartilhamento da lista de chamados iniciado com sucesso."); } else if (mounted) { print("Compartilhamento cancelado ou falhou: ${result.status}"); } } catch (e) { if(mounted) Navigator.of(context, rootNavigator: true).pop(); print("Erro ao gerar/compartilhar PDF da lista: $e"); if(mounted) ScaffoldMessenger.of(context).showSnackBar( SnackBar(content: Text('Erro ao gerar PDF da lista: ${e.toString()}')) ); } }
            ...
            builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
              ...
              return GridView.builder(
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    ...
                    // Extração de dados (adicionando telefone e técnico)
                    ...
                    // --- ADICIONADO ---
                    final String? creatorPhone = data['creatorPhone'] as String?; // Pega como String opcional
                    // !! TROQUE 'tecnico_designado' pelo nome real do campo no seu Firestore !!
                    final String? tecnicoResponsavel = data['tecnico_responsavel'] as String?; // Pega como String opcional
                    // ------------------
                    ...
                    // Instancia o TicketCard passando os novos dados
                    return TicketCard(
                      ...
                      creatorPhone: creatorPhone,       // <<< PASSANDO TELEFONE
                      tecnicoResponsavel: tecnicoResponsavel, // <<< PASSANDO TÉCNICO
