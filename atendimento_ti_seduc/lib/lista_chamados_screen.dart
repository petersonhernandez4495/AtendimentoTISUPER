...
import 'dart:math'; // Não parece ser usado, pode ser removido se não for necessário.
...
class _ListaChamadosScreenState extends State<ListaChamadosScreen> {
  ...
  final List<String> _statusOptions = [
    kStatusAberto, kStatusEmAndamento, kStatusPendente, kStatusPadraoSolicionado,
    kStatusCancelado, kStatusAguardandoAprovacao, kStatusAguardandoPeca,
    kStatusChamadoDuplicado, kStatusAguardandoEquipamento, kStatusAtribuidoGSIOR,
  ...
  final List<String> _statusAtivosRequisitante = [
    kStatusAberto, kStatusEmAndamento, kStatusPendente, kStatusAguardandoAprovacao,
    kStatusAguardandoPeca, kStatusChamadoDuplicado, kStatusAguardandoEquipamento,
    kStatusAtribuidoGSIOR, kStatusGarantiaFabricante,
  ];
    ...
    if (user != null) {
      ...
      try {
        ...
        if (userDoc.exists && userDoc.data() != null) {
          ...
          isAdminResult = (userData[kFieldUserRole] == 'admin'); // Constante importada
          if (!isAdminResult) {
            userInstitutionResult = userData[kFieldUserInstituicao] as String?; // Constante importada
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Erro ao verificar permissões do usuário: $e')),
          );
        }
      }
    } else {
  ...
  bool get _isFilterActive {
    return _selectedStatusFilter != null ||
        ...
        (_selectedSortOption['field'] != kFieldDataCriacao || _selectedSortOption['descending'] != true) || // Verifica se não é o sort padrão
  ...
  Query _buildFirestoreQuery() {
    ...
    if (!_isAdmin && _currentUser != null) {
      if (_currentUserInstitution == null || _currentUserInstitution!.isEmpty) {
        // Usuário sem instituição definida, vê apenas os seus próprios chamados
        ...
        // Usuário com instituição, vê seus chamados OU chamados da sua instituição
        query = query.where(
          Filter.or(
                            ...
                            TextButton(
                              onPressed: () {
                                ...
                                setState(() {
                                  ...
                                  _selectedDateFilter = null;
                                  _selectedSortOption = _sortOptions[0]; // Reset para o padrão
                                });
                              ...
                              child: OutlinedButton.icon(
                                ...
                                onPressed: () async {
                                  final DateTime? pickedDate = await showDatePicker(
                                    context: context, // Usar o context principal
                                  ...
                                  if (pickedDate != null) {
                                    ...
                                    sheetSetState(() {}); // Atualiza o estado do sheet
                                  }
                                },
                              ),
                            ),
  ...
  Future<void> _excluirChamado(BuildContext context, String chamadoId) async {
    ...
    bool confirmar = await showDialog<bool>(
          ...
          builder: (ctx) => AlertDialog(
            ...
            content: const Text('Deseja realmente excluir este chamado? Esta ação é irreversível.'),
            actions: [
    ...
    final scaffoldMessenger = ScaffoldMessenger.of(context); // Captura antes do async
    try {
      await _chamadoService.excluirChamado(chamadoId);
      if (mounted) {
        scaffoldMessenger.showSnackBar(const SnackBar(content: Text('Chamado excluído!'), backgroundColor: Colors.green));
      }
    } catch (e) {
      if (mounted) {
        scaffoldMessenger.showSnackBar(SnackBar(content: Text('Erro ao excluir: ${e.toString()}'), backgroundColor: Colors.red));
      }
    }
  }
            ...
            children: [
              TextButton.icon(
                icon: Icon(
                  ...
                  color: _isFilterActive ? colorScheme.primary : AppTheme.kWinSecondaryText, // Verifique AppTheme.kWinSecondaryText
                  size: 20,
                ),
                label: Text('Filtros',
                    style: TextStyle(
              ...
              : StreamBuilder<QuerySnapshot>(
                  ...
                  builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
                    if (snapshot.hasError) {
                      return Center(child: Padding(padding: const EdgeInsets.all(16.0), child: Text('Ocorreu um erro: ${snapshot.error}', textAlign: TextAlign.center)));
                    }
                    ...
                    if (snapshot.connectionState == ConnectionState.waiting &&
                        ...
                        widget.searchQuery.isEmpty && _selectedStatusFilter == null && _selectedDateFilter == null) {
                    ...
                    if (snapshot.hasData) {
                      chamadosDoFirestore = snapshot.data!.docs
                          .map((doc) {
                            try {
                              ...
                              return Chamado.fromFirestore(doc as DocumentSnapshot<Map<String, dynamic>>);
                            } catch (e) {
                      ...
                      // Se não tem dados e não está esperando, usa a lista cacheada (pode estar vazia)
                       _searchLogic.setChamadosSource(_ultimosChamadosFiltradosParaExibicao);
                    }


                    _searchLogic.filterChamadosComQuery(widget.searchQuery);
                    _ultimosChamadosFiltradosParaExibicao = _searchLogic.resultadosFiltrados;

                    if (_ultimosChamadosFiltradosParaExibicao.isEmpty) {
                      String msg = "Nenhum chamado encontrado.";
                      IconData icone = Icons.inbox_outlined;
                      if (_isFilterActive) {
                          msg = 'Nenhum chamado encontrado com os critérios aplicados.';
                          if (widget.searchQuery.isNotEmpty) {
                            msg += '\nPesquisa: "${widget.searchQuery}"';
                          }
                          icone = Icons.filter_alt_off_outlined;
                      } else if (!_isLoadingRole) { // Só define msg padrão se não estiver carregando role
                          msg = _isAdmin
                            ...
                            children: [
                              ...
                              if (_isFilterActive) ...[
                                ...
                                ElevatedButton.icon(
                                  ...
                                  label: const Text('Limpar Filtros e Pesquisa'),
                                  onPressed: () {
                                    setState(() {
                                      _selectedStatusFilter = null;
                                      _selectedDateFilter = null;
                    ...
                    return ListView.builder(
                      ...
                      itemBuilder: (BuildContext context, int index) {
                        final Chamado chamado = _ultimosChamadosFiltradosParaExibicao[index];
                        ...
                        if (snapshot.hasData) {
                          ...
                          } catch (e) { // firstWhere lança StateError se não encontrar
                            originalDoc = null;
                          }

                          if (originalDoc != null && originalDoc.exists && originalDoc.data() != null) {
                              chamadoDataMap = originalDoc.data() as Map<String, dynamic>;
                          }
                        }
                        ...
                        if (chamadoDataMap.isEmpty) {
                            ...
                            chamadoDataMap = {
                              'id': chamado.id, // Adicione o ID se ChamadoListItem precisar
                              'status': chamado.status,
                              'prioridade': chamado.prioridade,
                              'data_criacao': chamado.dataAbertura, // ou dataCriacao se for o nome no modelo
                              'nome_solicitante': chamado.nomeSolicitante,
                              'patrimonio': chamado.patrimonio,
                              'problema_selecionado': chamado.problemaSelecionado,
                              'equipamento_selecionado': chamado.equipamentoSelecionado,
                              'creatorUid': chamado.solicitanteUid, // ou creatorUid
                        ...
                        return ChamadoListItem(
                          key: ValueKey(chamadoId + (chamado.dataAtualizacao?.millisecondsSinceEpoch.toString() ?? chamado.dataAbertura.millisecondsSinceEpoch.toString())),
                          chamadoId: chamadoId,
                          ...
                          onConfirmar: (id) {
                            if (chamado.solicitanteUid == _currentUser?.uid) {
                               ...
                               ScaffoldMessenger.of(context).showSnackBar(
                                 const SnackBar(content: Text('Apenas o solicitante original pode confirmar este chamado.'), backgroundColor: Colors.orange),
                               );
                            }
                          },
                          ...
                          onNavigateToDetails: (id) {
                            ...
                            ).then((_) { if (mounted) setState((){}); }); // Atualiza a lista ao voltar
                          },
                          isLoadingPdfDownload: isLoadingPdfItem, // Necessita da variável de estado _idChamadoGerandoPdf
                          onGerarPdfOpcoes: (id, data) {
                            ...
                            print('Gerar PDF para $id não implementado neste exemplo.');
                          },
