...
class _NovoChamadoScreenState extends State<NovoChamadoScreen> {
  ...
  // REMOVIDO: Controller do nome
  // final _nomeController = TextEditingController();
  final _celularController = TextEditingController(); // Mantido
  ...
  void initState() {
    ...
    _preencherDadosUsuario(); // Não preenche mais o nome
  }

  @override
  void dispose() {
    // REMOVIDO: Dispose do nomeController
  ...
  Future<void> _carregarConfiguracoes() async {
    // Implementação da função _carregarConfiguracoes (com logs) mantida como antes
  ...
  // Preenche telefone do usuário logado (NÃO mais o nome)
  Future<void> _preencherDadosUsuario() async {
     final user = FirebaseAuth.instance.currentUser;
     if (user != null) {
       // REMOVIDO: Preenchimento do nomeController
       // Preenche Telefone
  ...
  // --- LÓGICA DE ENVIO (MODIFICADA para pegar nome do Auth) ---
  Future<void> _enviarChamado() async {
    ...
    // *** Pega o nome diretamente do usuário autenticado com fallback ***
    final String nomeFinalSolicitante = user.displayName?.trim().isNotEmpty ?? false
        ? user.displayName!.trim() // Usa o displayName se não for nulo/vazio/espaços
        : (user.email ?? "Usuário App (${user.uid.substring(0, 6)})"); // Senão, tenta usar o email, ou um placeholder com parte do UID
    ...
    // Monta os dados para o Firestore usando o nome obtido
    final dadosChamado = <String, dynamic>{
        kFieldTipoSolicitante: _tipoSelecionado,
        kFieldNomeSolicitante: nomeFinalSolicitante, // <<< USA O NOME OBTIDO DO AUTH
        ...
        'creatorName': nomeFinalSolicitante, // Nome usado como solicitante principal
        'creatorPhone': creatorPhone,
        'authUserDisplayName': user.displayName, // Nome original do Auth (pode ser null)
        'authUserEmail': user.email, // Email do Auth
    };
    ...
    try {
      ...
      if (mounted) {
        ...
        // Limpa controllers (exceto nome que foi removido)
        _celularController.clear(); _marcaModeloController.clear(); _patrimonioController.clear(); _tecnicoResponsavelController.clear();
        setState(() { // Reseta estado
             _tipoSelecionado = null; _equipamentoSelecionado = null; _internetConectadaSelecionado = null;
             _problemaSelecionado = null; _resetDependentFields(); _currentStep = 0;
        });
  ...
  Widget _buildStep1Content() {
    // Conteúdo do Passo 1 (sem alterações nesta refatoração)
    return Column( crossAxisAlignment: CrossAxisAlignment.stretch, children: <Widget>[
        _buildDropdown<String>( labelText: '', hintText: 'Selecione Escola ou Superintendência', value: _tipoSelecionado, items: _tipos, onChanged: (newValue) { if (_tipoSelecionado != newValue) { setState(() { _tipoSelecionado = newValue; _resetDependentFields(); }); } }, validator: (value) => value == null ? 'Selecione o tipo' : null, ),
  ...
  // Constrói o conteúdo para o Passo 2 (SEM o campo de nome)
  Widget _buildStep2Content() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: <Widget>[
         // *** CAMPO DE NOME REMOVIDO DAQUI ***

         // Mantém o campo de celular e os demais
         _buildTextFormField(
           controller: _celularController,
           labelText: 'Número de celular para contato*',
           hintText: '(XX) XXXXX-XXXX', // Hint útil mantido
         ...
         _buildTextFormField(
           ...
           labelText: 'Marca/Modelo do equipamento? (Opcional)', // Label ligeiramente ajustado
           hintText: 'Ex: Dell Optiplex 3080, Positivo N250i', // Hint útil mantido
         ),
         const SizedBox(height: 16.0),
         _buildTextFormField(
           controller: _patrimonioController,
           labelText: 'Número de Patrimônio (Tombamento)?*', // Label ligeiramente ajustado
           hintText: 'Digite apenas números', // Hint útil mantido
           ...
           helperText: 'Caso não encontre a etiqueta, solicite à direção/secretaria a consulta no sistema.', // Helper text ajustado
           helperMaxLines: 3,
         ),
         const SizedBox(height: 16.0),
         _buildDropdown<String>(
         ...
         _buildTextFormField(
           ...
           hintText: 'Se souber, indique um técnico', // Hint útil mantido
         ),
         const SizedBox(height: 8.0),
      ],
    );
  ...
  // (Implementações que usam o tema, como na versão anterior)
