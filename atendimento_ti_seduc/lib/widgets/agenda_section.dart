    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshotVisitas) {
        if (snapshotVisitas.hasError) {
          return Text("Erro ao carregar agenda: ${snapshotVisitas.error}", style: const TextStyle(color: Colors.red));
        }
        ...
        if (!snapshotVisitas.hasData || snapshotVisitas.data!.docs.isEmpty) {
          return const Padding( padding: EdgeInsets.symmetric(vertical: 15.0), child: Center(child: Text("Nenhuma visita agendada.", style: TextStyle(color: Colors.grey))), );
        }
          ...
          children: snapshotVisitas.data!.docs.map((docVisita) {
              ...
              return Card(
                 ...
                 child: ListTile(
                   ...
                   title: Text("Agendado: $dataHoraAgendada", style: const TextStyle(fontWeight: FontWeight.w500)),
                   subtitle: Column( crossAxisAlignment: CrossAxisAlignment.start, children: [
                       const SizedBox(height: 3),
                       if (tecnico != 'Não definido') Text("Técnico: $tecnico"),
                       if (obs.isNotEmpty) Text("Obs: $obs", style: const TextStyle(fontStyle: FontStyle.italic)),
