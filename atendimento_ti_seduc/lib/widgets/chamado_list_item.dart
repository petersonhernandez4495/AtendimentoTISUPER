...
typedef ChamadoDataActionCallback = void Function(
    String chamadoId, Map<String, dynamic> chamadoData);
...
class ChamadoListItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Color textoSecundarioCor =
    ...
    if (titulo.isEmpty) {
      titulo = chamadoData[kFieldEquipamentoSolicitacao] as String? ??
          'Problema não especificado';
      if (titulo.toLowerCase() == 'outro') {
        titulo = chamadoData[kFieldEquipamentoOutro] as String? ??
            'Problema não especificado';
      }
    }

    final String prioridade =
        chamadoData[kFieldPrioridade]?.toString() ?? 'Média';
    final String status = chamadoData[kFieldStatus]?.toString() ?? 'N/I';
    final String creatorName = chamadoData['creatorName']?.toString() ??
        chamadoData[kFieldNomeSolicitante]?.toString() ??
        'N/I';
    ...
    final String? tecnicoResponsavel =
        chamadoData[kFieldTecnicoResponsavel] as String?;
    ...
    final bool isSolucionado =
        status.toLowerCase() == statusSolucionadoComparacao.toLowerCase();
    final bool isFinalizado =
        status.toLowerCase() == statusFinalizadoComparacao.toLowerCase();
    ...
    final String? instituicaoManualEscola =
        chamadoData[kFieldInstituicaoManual] as String?;
    final String? setorSuper = chamadoData[kFieldSetorSuper] as String?;
    final String? cidadeSuper =
        chamadoData[kFieldCidadeSuperintendencia] as String?;

    if (tipoSolicitante == 'ESCOLA') {
      if (cidadeEscola == "OUTRO" &&
          instituicaoManualEscola != null &&
          instituicaoManualEscola.isNotEmpty) {
      ...
      localPrincipalDisplay =
          instituicaoEscola ?? instituicaoManualEscola ?? setorSuper ?? "";
      localSecundarioDisplay = cidadeEscola ?? cidadeSuper;
    }

    String instituicaoCompleta = localPrincipalDisplay ?? "";
    if (localSecundarioDisplay != null &&
        localSecundarioDisplay.isNotEmpty &&
        localSecundarioDisplay != localPrincipalDisplay) {
    ...
    final bool isRequerente =
        currentUserId != null && currentUserId == creatorUid;
    ...
    final bool podeGerarPdf =
        isAdmin || (isRequerente && requerenteConfirmou && !isInativo);
          ...
          onTap: isClickable
              ...
              : () {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content:
                          const Text('Chamado inativo. Não pode ser aberto.'),
            ...
            children: [
              Padding(
                padding: const EdgeInsets.fromLTRB(
                    12.0, 10.0, 0.0, 10.0), // Reduzido padding direito
                child: Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Container(
                      ...
                      decoration: BoxDecoration(
                          color:
                              isInativo ? Colors.grey.shade400 : corPrioridade,
                        ...
                        children: [
                          Text(
                            ...
                            style: textTheme.titleMedium?.copyWith(
                              ...
                              decoration:
                                  isInativo ? TextDecoration.lineThrough : null,
                            ),
                            maxLines: 2,
                              ...
                              Expanded(
                                ...
                                child: Text(
                                  ...
                                  style: textTheme.bodySmall
                                      ?.copyWith(color: textoSecundarioCor),
                                  maxLines: 2,
                                  overflow: TextOverflow.ellipsis,
                                ),
                              ),
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      ...
                                      if (mostrarSolucaoAceitaChip) ...[
                                        ...
                                        Tooltip(
                                          message:
                                              'Solução aceita pelo requerente',
                                          child: Chip(
                                            avatar: Icon(
                              ...
                              child: Row(
                                ...
                                children: [
                                  Icon(Icons.engineering_outlined,
                                      size: 11, color: textoSecundarioCor),
                    ...
                    PopupMenuButton<String>(
                      icon: Icon(Icons.more_vert,
                          color: textoSecundarioCor), // Ícone movido para fora
                      tooltip: "Mais opções",
                      onSelected: (value) {
                        if (value == 'details') {
                          onNavigateToDetails(chamadoId);
                            ...
                            onDelete != null) {
                          onDelete!();
                        } else if (value == 'pdf_opcoes' &&
                            onGerarPdfOpcoes != null &&
                            podeGerarPdf) {
                          // <<-- CHAMADA DA NOVA CALLBACK
                          onGerarPdfOpcoes!(chamadoId, chamadoData);
                        } else if (value == 'confirmar_servico' &&
                            podeConfirmar &&
                            onConfirmar != null) {
                          onConfirmar!(chamadoId);
                        } else if (value == 'finalizar_arquivar' &&
                            podeFinalizarPelaLista &&
                            onFinalizarArquivar != null) {
                      ...
                      itemBuilder: (BuildContext context) {
                        ...
                        if (onGerarPdfOpcoes != null) {
                          items.add(PopupMenuItem<String>(
                              value: 'pdf_opcoes',
                              enabled: podeGerarPdf &&
                                  !isLoadingPdfDownload, // Habilita baseado na condição e loading
                              ...
                              child: isLoadingFinalizarArquivar
                                  ...
                                  : ListTile(
                                      leading: Icon(Icons.archive_outlined,
                                          size: 20,
                                          color: colorScheme.onSurfaceVariant),
                                      title: Text('Finalizar e Arquivar',
                                          style: textTheme.bodyMedium),
                                      dense: true,
                                      contentPadding: EdgeInsets.zero)));
                        }
              ...
              if (!isInativo) ...[
                if (podeConfirmar)
                  Padding(
                    padding: const EdgeInsets.fromLTRB(
                        12.0, 0.0, 12.0, 10.0), // Ajustado padding
                    child: Column(
                      children: [
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            solucao,
                            style: textTheme.bodySmall?.copyWith(
                                color: textoSecundarioCor.withOpacity(0.9)),
                            maxLines: 2,
                            overflow: TextOverflow.ellipsis,
                          ),
                        ],
